  ...
  void onQueryChanged(String query) {
    ...
    for (int i = 0; i < tableObjsJson.length; i++) {
      ...
      if (myString.contains(_search.text)) {
        tableTemp.add(
            {"n_id": tableObjsJson[i]["n_id"], "name": tableObjsJson[i]["name"], "age": tableObjsJson[i]["age"], "birthday": tableObjsJson[i]["birthday"]});
      }
    }
    print(tableTemp);
    setState(() {
...
class _ListBuilderState extends State<ListBuilderState> {
  Future<String> download() async {
    if (tableObjsJson.length > 1) {
      ...
      var request = await HttpClient().getUrl(Uri.parse('https://anchih.e-rec.ru/api/'));
      // sends the request
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return Card(
                      child: ListTile(
                        ...
                        subtitle: Text(
                          "${"Возрасть:" + tableTemp[index]["age"]} год.рождения:" + tableTemp[index]["birthday"],
                        ),
                        trailing: Row(
                              ...
                              onPressed: () {
                                showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return add_person(tableTemp[index]["name"], tableTemp[index]["n_id"]);
                                    });
                              },
                            ),
                            IconButton(
...
class _alert_list extends StatelessWidget {
  ...
  Future<String> get_person() async {
    var request = await HttpClient().getUrl(Uri.parse('https://anchih.e-rec.ru/api/person?id=$id_person'));
    // sends the request
    var response = await request.close();
    // transforms and prints the response
    await for (var contents in response.transform(const Utf8Decoder())) {
                                  ...
                                  InkWell(
                                    ...
                                    onTap: () {
                                      showDialog(
                                          context: context,
                                          builder: (BuildContext context) {
                                            return _alert_list(obj_person[i]["n_id"]);
                                          });
                                    },
                                  ),
                                ],
...
class _add_person extends State<add_person> {
  ...
  Future<String> getFamaly() async {
    var request = await HttpClient().getUrl(Uri.parse('https://anchih.e-rec.ru/api/person/getfamaly.php?id=$id_person'));
    // sends the request
    var response = await request.close();
    // transforms and prints the response
    await for (var contents in response.transform(const Utf8Decoder())) {
      ...
      fams = obj_femaly.map((item) => [item['name'].toString(), item['FAMC'].toString()]).toList();
    }
    return Future.value("Data download"); // return your response
  }

  Future<void> _sendDataToServer(String id_famaly, String name, String surname, String age, String relation) async {
    final url = Uri.parse('https://anchih.e-rec.ru/api/person/add_person.php'); // Замените на ваш URL
                  ...
                  TextButton(
                    onPressed: () {
                      if (selectedFams == null) {
                        _showErrorDialog('Пожалуйста, выберите мать для ребенка');
                        return;
                      }
